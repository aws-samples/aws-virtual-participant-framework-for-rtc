...
def start_instance(event):
    ...
    response = ec2.run_instances(
        ...
        # KeyName = key_name,
        IamInstanceProfile={"Arn": BUILDER_INSTANCE_PROFILE_ARN},
        TagSpecifications=[
            {
                "ResourceType": "instance",
                "Tags": [{"Key": "AmiBuilder", "Value": "True"}],
            }
        ],
    )
    ...
    return {"instanceId": instance_id, "status": STATUS_IN_PROGRESS}
...
def stop_instance(event):
    ...
    return {"instanceId": instance_id, "status": get_instance_status(state_name)}
