...
def lambda_handler(event, context):
    ...
    try:
        ...
        for job in jobs:
            ...
            print(
                f"processing job: {job_id} with continuationToken: {continuation_token}"
            )
...
def start_job_flow(job_id, job):
    ...
    configuration = get_job_attribute(job, "actionConfiguration", {}).get(
        "configuration", {}
    )
